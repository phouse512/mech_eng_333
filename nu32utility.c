...
// call this program with 2 arguments: the name of the serial port, and the location of the hex file
int main(int argc, char *argv[]) {
  printf("\r\n=== NU32 Serial Bootloader   check cr nl ===\r\n");
  ...
  int i=0,j=0,k=0;
  char hex[2];
  char message[50]; // SOH+0x03+[HEX]+CRCL+CRCH+EOT 
  char message_buffer[52]; // extra bytes needed for special characters
  int CRC;
  ...
  while(percent < lines) {
    ...
    j++;
    for (i=1;i<length;i=i+2) { // start at 1 to skip the ':' at beginning of each line
      hex[0] = array[i];
      hex[1] = array[i+1];
